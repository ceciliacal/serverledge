...
func TestCo2QosEvaluateLocalExec(t *testing.T) {
	...
	p := testPolicy(t, fun)
	p.probabilityCache.Store("local|standard", MultiRegionProbs{PLocal: 1, PCloud: map[string]float64{}})
...
func TestCo2QosEvaluateOffloadEdge(t *testing.T) {
	...
	p := testPolicy(t, fun)
	p.probabilityCache.Store("edge|standard", MultiRegionProbs{PEdge: 1, PCloud: map[string]float64{}})
...
func TestCo2QosEvaluateOffloadCloud(t *testing.T) {
	...
	p := testPolicy(t, fun)
	p.probabilityCache.Store("cloud|standard", MultiRegionProbs{PCloud: map[string]float64{"cloud_a": 1}})
...
func TestCo2QosEvaluateDrop(t *testing.T) {
	...
	p := testPolicy(t, fun)
	p.probabilityCache.Store("drop|standard", MultiRegionProbs{PDrop: 1, PCloud: map[string]float64{}})
...
func TestParseOptimizerResponseErrors(t *testing.T) {
	tests := []struct {
		...
		{name: "exec var excluded", body: `{"f":{"standard":{"EXEC_VAR":1}}}`},
	}
...
func TestParseOptimizerResponseAcceptsRealServerWrapper(t *testing.T) {
	...
	_, err = parseOptimizerResponse([]byte(`{"probs":{"f":{"standard":{"EXEC":1}}},"decision_fc_probability_var":{"f":{"f_fast":{"standard":0.5}}}}`))
	if err == nil {
		t.Fatal("parseOptimizerResponse() accepted variant probabilities")
	}
}
...
func TestOptimizerUnavailableDoesNotPopulateCache(t *testing.T) {
	...
	if _, ok := p.probabilityCache.Load("unavailable|standard"); ok {
...
func TestInvokeOptimizerPopulatesProbabilityCache(t *testing.T) {
	...
	got, ok := p.probabilityCache.Load("optimizer-success|standard")
	if !ok {
		t.Fatal("probability cache missing optimizer entry")
	}
	probs := got.(MultiRegionProbs)
	if probs.PLocal != 1 {
		t.Fatalf("PLocal = %v, want 1", probs.PLocal)
	}
}
...
func TestCo2QosWorkflowStageUsesCurrentSchedulerRequest(t *testing.T) {
	...
	req := testScheduledRequest(fun)
	req.Request.Ctx = nil
	p.probabilityCache.Store("workflow-stage|standard", MultiRegionProbs{PDrop: 1, PCloud: map[string]float64{}})
...
func TestCo2QosPrepareParamsIgnoresVariantMetadata(t *testing.T) {
	...
	if len(params.Functions) != 1 || params.Functions[0] != "base-fast" {
		t.Fatalf("Functions = %#v", params.Functions)
	}
	payloadType := reflect.TypeOf(params)
	if _, ok := payloadType.FieldByName("Variants"); ok {
		t.Fatal("optimizer params contain variant enumeration")
	}
	if _, ok := payloadType.FieldByName("VariantUtility"); ok {
		t.Fatal("optimizer params contain variant utility")
	}
}
...
func TestOptimizerRequestContractMatchesRealPythonOptimizer(t *testing.T) {
	...
	if _, ok := raw["variants"]; ok {
		t.Fatal("payload contains variant-specific variants field")
	}
	if _, ok := raw["variant_utility"]; ok {
		t.Fatal("payload contains variant-specific variant_utility field")
	}
...
if variants:
    raise SystemExit("variant probabilities returned for phase 1")
print("ok")
`
...
func assertPhase1OptimizerContract(t *testing.T, params OptCarbonAwareParams) {
	t.Helper()
	for _, decision := range params.PossibleDecisions {
		if decision == "EXEC_VAR" {
			t.Fatal("possible_decisions contains EXEC_VAR")
		}
	}
	...
	for name, got := range requiredFunctionMaps {
		if got != len(params.Functions) {
			t.Fatalf("%s has %d functions, want %d", name, got, len(params.Functions))
		}
	}
