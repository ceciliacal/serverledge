...
def plot_csv_data(csv_file, battery_file_path, start_ts, end_ts, function_name, minutes_to_add: int):
    ...
    ax1.set_ylabel('Throughput [req/sec]', fontsize=20)

    ax1_x_values = len(resampled_df['timestamp'])
    print(ax1_x_values)

    if "baseline" in function_name:
        rows_to_add = int((60 / 5) * minutes_to_add)
    ...
    ax2.set_ylabel('Battery [%]', fontsize=20)
    ax2.yaxis.tick_right()
    ax2.yaxis.set_label_position('right')
    ax2.set_xticks([])
    ax2.set_yticks(np.arange(0, 101.0, step=10))
    ax2.set_ylim(bottom=0)

    h1, l1 = ax1.get_legend_handles_labels()
    h2, l2 = ax2.get_legend_handles_labels()
    ax1.legend(h1 + h2, l1 + l2, fontsize=20, loc=0)
