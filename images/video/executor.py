...
class Executor(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        try:
            ...
            context["EnergySavingSoC"] = request["Context"]["EnergySavingSoC"]
        except Exception:
            pass

        if handler_dir not in added_dirs:
        ...
        try:
            ...
            u1, s1, cu1, cs1, _ = os.times()  # include children times

            # Call function
            result = getattr(mod, func_name)(params, context)

            wall_end = time.time()
            u2, s2, cu2, cs2, _ = os.times()

            wall = wall_end - wall_start
            cpu_time = (u2 - u1) + (s2 - s1) + (cu2 - cu1) + (cs2 - cs1)
            ...
            response["CPUTime"] = cpu_time
            response["WallTime"] = wall
